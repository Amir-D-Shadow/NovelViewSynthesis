...
def train_one_epoch(images_data, H, W, ray_params, opt_nerf, opt_focal,opt_pose,opt_time, nerf_model, focal_net, pose_param_net,time_pose_net):
    ...
    for t in timeSteps:
        ...
        for i in ids:
            ...
            r_id = torch.randperm(H, device=cuda_id)[:64]  # (N_select_rows)
            c_id = torch.randperm(W, device=cuda_id)[:64]  # (N_select_cols)
...
#training
N_EPOCH = 7000  # set to 1000 to get slightly better results. we use 10K epoch in our paper.
...
if load_model:
    ...
    focal_net = focal_net.cuda(int(cuda_id[-1]))
    ...
    pose_param_net = pose_param_net.cuda(int(cuda_id[-1]))
    ...
    time_pose_net = time_pose_net.cuda(int(cuda_id[-1]))

else:

    # Initialise all trainabled parameters
    focal_net = LearnFocal(H, W, req_grad=True).cuda(int(cuda_id[-1]))
    pose_param_net = LearnPose(num_cams=N_IMGS, learn_R=True, learn_t=True).cuda(int(cuda_id[-1]))
    time_pose_net = LearnTime(in_feat=3,mid_feat=32,out_feat=3).cuda(int(cuda_id[-1]))


# Get a tiny NeRF model. Hidden dimension set to 128
nerf_model = TinyNerf(pos_in_dims=63, dir_in_dims=27,time_in_dim=27, D=312).cuda(int(cuda_id[-1]))
